...
from config import DB_NAME # This will import DB_NAME from src/config.py
...
for trade_id in trade_ids_to_rehydrate:
    try:
        ...
        if trade_data:
            ...
            logger.info(f"Trade {trade_id}: Successfully restored quantity='{quantity}' and status='{status}' in Redis.")
        else:
            logger.warning(f"Trade {trade_id}: Not found in SQLite database. Skipping rehydration.")

...
def main():
    ...
    p.add_argument(
        "--redis", default=os.environ.get("REDIS_URL"), help="Redis URL (env REDIS_URL)"
    )
    ...
    if found == 0:
        print(
            "No keys matched the pattern. Consider broadening the pattern or checking DB-backed settings."
        )
    else:

...
def main():
    ...
    p.add_argument('--db', default=os.environ.get('DB_PATH', 'lunara_bot.db'), help='Path to sqlite DB')
    ...
    if args.user:
        ...
        if 'user_id' in cols:
            ...
            print('Warning: table does not contain a user_id column; returning all rows')
    ...
    for r in rows:
        ...
        if 'quantity' not in rowd:
            ...
            q = rowd.get('quantity')
            if q is None or (isinstance(q, (int, float)) and q == 0) or (isinstance(q, str) and q.strip() == ''):
                print('>> PROBLEM: quantity is empty or zero for trade id=', rowd.get('id'))
                problems += 1

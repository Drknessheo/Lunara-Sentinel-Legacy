...
async def main():
    ...
    created = await force_create_mock_slips(
        user_id=user_id, symbols=symbols, context=None, max_create=2
    )
    ...
    created_display = (
        ",".join(created) if isinstance(created, list) and created else str(created)
    )
    print(
        f'- {ts_fmt} by {admin_name} result={audit_entry.get("result")} created={created_display}'
    )

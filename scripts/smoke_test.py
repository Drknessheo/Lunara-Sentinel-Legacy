...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Smoke test health + promotion webhook"
    )
    parser.add_argument(
        "--base-url",
        default=os.environ.get("SMOKE_BASE_URL", "http://localhost:8080"),
        help="Base URL where the bot/health server is reachable",
    )
    parser.add_argument(
        "--health-path",
        default=os.environ.get("SMOKE_HEALTH_PATH", "/health"),
        help="Health endpoint path (default /health)",
    )
    parser.add_argument(
        "--webhook-url",
        default=os.environ.get("PROMOTION_WEBHOOK_URL", "https://httpbin.org/post"),
        help="Promotion webhook URL to POST a test payload to",
    )
    parser.add_argument(
        "--webhook-secret",
        default=os.environ.get("PROMOTION_WEBHOOK_SECRET"),
        help="Optional HMAC secret to sign the test webhook",
    )
    parser.add_argument(
        "--timeout", type=float, default=5.0, help="Request timeout (seconds)"
    )
    ...
    ok2, wstatus, wtext = run_webhook_test(
        args.webhook_url, args.webhook_secret, args.timeout
    )

...
def migrate_schema():
    ...
    if "mode" not in trade_columns:
        logger.info(
            "Migrating database: Adding 'mode' and 'trade_size_usdt' columns to 'trades' table."
        )
    ...
    if "close_reason" not in trade_columns:
        logger.info(
            "Migrating database: Adding 'close_reason' column to 'trades' table."
        )
    ...
    if "win_loss" not in trade_columns:
        logger.info(
            "Migrating database: Adding 'win_loss' and 'pnl_percentage' columns to 'trades' table."
        )
    ...
    if "dsl_mode" not in trade_columns:
        logger.info(
            "Migrating database: Adding 'dsl_mode' and 'current_dsl_stage' columns to 'trades' table."
        )
        cursor.execute("ALTER TABLE trades ADD COLUMN dsl_mode TEXT")
        cursor.execute(
            "ALTER TABLE trades ADD COLUMN current_dsl_stage INTEGER DEFAULT 0"
        )
        changes_made = True
    ...
    if "trading_mode" not in user_columns:
        logger.info(
            "Migrating database: Adding 'trading_mode' and 'paper_balance' columns to 'users' table."
        )
        cursor.execute("ALTER TABLE users ADD COLUMN trading_mode TEXT DEFAULT 'LIVE'")
        cursor.execute(
            "ALTER TABLE users ADD COLUMN paper_balance REAL DEFAULT 10000.0"
        )
        changes_made = True

    if "custom_stop_loss" not in user_columns:
        logger.info(
            "Migrating database: Adding custom setting columns to 'users' table."
        )
    ...
    if "custom_bollinger_band_width" not in user_columns:
        logger.info(
            "Migrating database: Adding custom bollinger band and macd signal columns to 'users' table."
        )
...
def get_user_effective_settings(user_id: int) -> dict:
    ...
    settings = config.get_active_settings(
        tier
    ).copy()  # Start with a copy of tier defaults
    conn = get_db_connection()
    ...
    if (
        "custom_trailing_activation" in user_keys
        and user_data["custom_trailing_activation"] is not None
    ):
        settings["TRAILING_PROFIT_ACTIVATION_PERCENT"] = user_data[
            "custom_trailing_activation"
        ]
    if (
        "custom_trailing_drop" in user_keys
        and user_data["custom_trailing_drop"] is not None
    ):
        settings["TRAILING_STOP_DROP_PERCENT"] = user_data["custom_trailing_drop"]
    if (
        "custom_bollinger_band_width" in user_keys
        and user_data["custom_bollinger_band_width"] is not None
    ):
...
def get_open_trades(user_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, user_id, coin_symbol, buy_price, buy_timestamp, stop_loss_price, take_profit_price FROM trades WHERE user_id = ? AND status = 'open'",
        (user_id,),
    )
...
def get_open_trade_by_symbol(user_id: int, symbol: str):
    ...
    conn = get_db_connection()
    return conn.execute(
        "SELECT * FROM trades WHERE user_id = ? AND coin_symbol = ? AND status = 'open'",
        (user_id, symbol),
    ).fetchone()
...
def close_trade(
    ...
    trade = conn.execute(
        "SELECT buy_price, coin_symbol FROM trades WHERE id = ? AND user_id = ? AND status = 'open'",
        (trade_id, user_id),
    ).fetchone()
    if not trade:
        return False
    ...
    if win_loss is None:
        win_loss = (
            "win"
            if pnl_percentage > 0
            else ("loss" if pnl_percentage < 0 else "break_even")
        )
    ...
    cursor.execute(
        ...
        (
            coin_symbol,
            1 if win_loss == "win" else 0,
            1 if win_loss == "loss" else 0,
            pnl_percentage,
            1 if win_loss == "win" else 0,
            1 if win_loss == "loss" else 0,
            pnl_percentage,
...
def update_trade_stop_loss(
    trade_id: int, new_stop_loss_price: float, new_dsl_stage: int
):
...
def update_trade_field(trade_id, field_name, value):
    ...
    try:
        ...
        conn.execute(
            f"UPDATE trades SET {field_name} = ? WHERE id = ?", (value, trade_id)
        )
        conn.commit()
    except sqlite3.Error as e:
        logger.error(
            f"Database error updating field {field_name} for trade {trade_id}: {e}"
        )
...
def get_unique_open_trade_symbols():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT coin_symbol FROM trades WHERE status = 'open'")
...
def set_user_trading_mode(user_id: int, mode: str):
    ...
    conn = get_db_connection()
    conn.execute(
        "UPDATE users SET trading_mode = ? WHERE user_id = ?", (mode.upper(), user_id)
    )
    conn.commit()
...
def get_all_user_ids() -> list[int]:
    ...
    conn = get_db_connection()
    user_ids = [
        row["user_id"] for row in conn.execute("SELECT user_id FROM users").fetchall()
    ]
    return user_ids
...
def get_autotrade_status(user_id: int) -> bool:
    ...
    conn = get_db_connection()
    row = conn.execute(
        "SELECT autotrade_enabled FROM users WHERE user_id = ?", (user_id,)
    ).fetchone()
...
def get_premium_users() -> list[int]:
    ...
    user_ids = [
        row["user_id"]
        for row in conn.execute("SELECT user_id FROM premium_users").fetchall()
    ]
    return user_ids
...
def get_coin_performance(coin_symbol: str):
    ...
    conn = get_db_connection()
    return conn.execute(
        "SELECT * FROM coin_performance WHERE coin_symbol = ?", (coin_symbol,)
    ).fetchone()

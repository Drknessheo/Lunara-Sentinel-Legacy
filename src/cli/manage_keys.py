...
def verify_backups(backup_service):
    ...
    try:
        ...
        backup_files = [
            f for f in os.listdir(backup_service.backup_dir) if f.endswith(".bak")
        ]
...
def main():
    ...
    group.add_argument(
        "--status", action="store_true", help="Check key rotation status"
    )
    group.add_argument(
        "--rotate-now", action="store_true", help="Perform immediate key rotation"
    )
    group.add_argument(
        "--verify-backup", action="store_true", help="Verify all key backups"
    )
    group.add_argument(
        "--restore-backup", type=str, help="Restore from specific backup file"
    )

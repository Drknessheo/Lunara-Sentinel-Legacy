...
def generate_daily_summary():
    ...
    if not BOT_TOKEN or not CREATOR_ID:
        logging.error(
            "TELEGRAM_BOT_TOKEN or TELEGRAM_CREATOR_ID not found in .env file."
        )
        return
    ...
    try:
        ...
        send_telegram_message(
            BOT_TOKEN,
            CREATOR_ID,
            "LunessaSignals Report: Analytics file not found or is empty.",
        )
        return
    ...
    yesterday_stats = data.get("daily_stats", {}).get(
        yesterday_str, {"interactions": 0, "earnings": 0.0}
    )

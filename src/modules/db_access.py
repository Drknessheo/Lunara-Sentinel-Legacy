...
def update_user_setting(cursor, user_id: int, setting_name: str, value):
    ...
    if value is None:
        cursor.execute(
            f"UPDATE users SET {column} = NULL WHERE user_id = ?", (user_id,)
        )
    else:
        cursor.execute(
...
def get_estimated_audit_rows(cursor, limit: int = 10, offset: int = 0):
    ...
    rows = cursor.execute(
        "SELECT * FROM estimated_quantities_audit ORDER BY created_at DESC LIMIT ? OFFSET ?",
        (limit, offset),
    ).fetchall()
    return rows
...
def promote_estimate_to_trade(cursor, audit_id: int) -> tuple[bool, str]:
    ...
    audit_row = cursor.execute(
        "SELECT * FROM estimated_quantities_audit WHERE id = ?", (audit_id,)
    ).fetchone()
    if not audit_row:
    ...
    cursor.execute(
        "UPDATE estimated_quantities_audit SET promoted = 1 WHERE id = ?", (audit_id,)
    )
...
def get_autotrade_status(cursor, user_id: int):
    ...
    return (
        bool(row["autotrade_enabled"])
        if row and row["autotrade_enabled"] is not None
        else False
    )
...
def update_trade_stop_loss(cursor, trade_id: int, new_stop_loss: float):
    ...
    cursor.execute(
        "UPDATE trades SET stop_loss_price = ? WHERE id = ?", (new_stop_loss, trade_id)
    )
...
def update_dsl_stage(cursor, trade_id: int, new_stage: int):
    ...
    cursor.execute(
        "UPDATE trades SET current_dsl_stage = ? WHERE id = ?", (new_stage, trade_id)
    )
...
def log_trade(
    ...
    if trade_size_usdt is not None and trade_size_usdt < 5:
        logger.warning(
            f"Trade for user {user_id} on {coin_symbol} below notional threshold: {trade_size_usdt}"
        )
        return

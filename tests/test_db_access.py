...
def test_get_user_effective_settings(setup_test_db, monkeypatch):
    ...
    monkeypatch.setattr(db_access.config, 'get_active_settings', lambda tier: {'RSI_BUY_THRESHOLD': 30, 'RSI_SELL_THRESHOLD': 70})
...
def test_api_key_encryption_decryption(setup_test_db):
    ...
    cursor.execute("INSERT INTO users (user_id, api_key, secret_key) VALUES (?, ?, ?)", (user_id, api_key, secret_key))
...
def test_watchlist_functions(setup_test_db):
    ...
    db_access.get_or_create_user(cursor, user_id)
    # Add to watchlist
    cursor.execute("INSERT INTO watchlist (user_id, coin_symbol) VALUES (?, ?)", (user_id, 'BTCUSDT'))
...
def test_trade_functions(setup_test_db):
    ...
    db_access.get_or_create_user(cursor, user_id)
    # Add a trade
    cursor.execute("INSERT INTO trades (user_id, coin_symbol, buy_price, status) VALUES (?, ?, ?, ?)", (user_id, 'BTCUSDT', 50000, 'open'))

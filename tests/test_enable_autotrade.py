...
SCRIPT_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "scripts", "enable_autotrade.py")
)
...
def test_falls_back_to_lunessa(monkeypatch):
    ...
    sys.modules["src.Lunessa_db"] = make_module(
        "src.Lunessa_db", set_autotrade_status=setter2
    )
...
def test_falls_back_to_modules_db_access(monkeypatch):
    ...
    sys.modules["src.modules.db_access"] = make_module(
        "src.modules.db_access", set_autotrade_status=setter3
    )
    ...
    def _block(name, globals=None, locals=None, fromlist=(), level=0):
        if (
            name in ("src.db", "src.Lunessa_db")
            or name.startswith("src.db.")
            or name.startswith("src.Lunessa_db.")
        ):
            raise ImportError
        return orig_import(name, globals, locals, fromlist, level)
